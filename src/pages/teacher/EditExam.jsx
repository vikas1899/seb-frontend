...
const EditExam = () => {
  const { id } = useParams()
  const { exam, loading: examLoading, error: examError, refetch } = useExam(id)
  const { updateExam, uploadSEBConfig } = useExams()
  ...
  useEffect(() => {
    if (exam) {
      ...
      setFormData({
        ...
        sebFile: null, // File input always starts empty
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form (SEB file is optional for updates)
    const validationData = { ...formData, id: exam.id }
    const validation = validateExamForm(validationData)
    ...
    try {
      ...
      if (result.success) {
        // Upload new SEB config file if provided
        if (formData.sebFile) {
          ...
          if (!uploadResult.success) {
            toast.error('Exam updated but SEB config upload failed')
          }
        }
  ...
  if (examError) {
    return (
      <div className="min-h-screen flex flex-col bg-gray-50">
        <Header />
        <div className="flex-1 flex items-center justify-center">
          <ErrorMessage error={examError} onRetry={refetch} />
        </div>
        <Footer />
      </div>
    )
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Update your exam configuration</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="card">
            <div className="card-header">
              ...
              <p className="card-description">
                Update when the exam will be available and how long it will last
              </p>
            </div>
            <div className="card-content space-y-6">
          ...
          <div className="card">
            <div className="card-header">
              ...
              <p className="card-description">
                Upload a new Safe Exam Browser configuration file (optional)
              </p>
            </div>
            <div className="card-content">
              <div>
                <label htmlFor="sebFile" className="block text-sm font-medium text-gray-700 mb-2">
                  New SEB Config File (.seb)
                </label>
                <input
                  id="sebFile"
                  name="sebFile"
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Leave empty to keep the current SEB configuration
                </p>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <p className="card-description">
                Update which system checks students must pass before taking the exam
              </p>
            </div>
            <div className="card-content">
          ...
          <div className="card">
            <div className="card-header">
              ...
              <p className="card-description">
                Update instructions that will be shown to students before the exam
              </p>
            </div>
            <div className="card-content">
              <textarea
            ...
            >
              {isSubmitting ? (
                <div className="flex items-center">
                  ...
                  <span className="ml-2">Updating Exam...</span>
                </div>
              ) : (
                <>
